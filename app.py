...
# Módulo 12: Lógica de Performance/Cache do Item 8
...
# --- NOVAS CONSTANTES DE ESTILO ---
STYLE_FONT_SIZE = '18px' # Tamanho da fonte MAIOR para tabelas e matrizes
...
# Inicialização do Estado para Pesquisa no Item 9
...
if not df_dados_original.empty:
    ...
    df_desempenho_filtrado = calcular_metricas_temporais(
        ...
        df_novos_cadastrados_original, # <--- ALTERAÇÃO: Passando o DF de cadastros
    ...
    if not df_combinado.empty:
        ...
        st.info("Nenhum dado encontrado para o comparativo de desempenho na(s) temporada(s) selecionada(s).")
    ...
    if not df_exibir_categorias.empty:
        st.dataframe(
            df_exibir_categorias[['Profissional Ativo', 'Qtd Segmento', 'Qtd Gabriel Pro', 'Participacao Texto']].style
                ...
                .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['Qtd Segmento', 'Qtd Gabriel Pro', 'Participacao Texto']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
            use_container_width=True,
            column_config={
        ...
        st.info("Nenhuma categoria encontrada para os filtros selecionados.")
    ...
    if 'Mês_Exibicao' in df_filtrado.columns and 'Temporada_Exibicao' in df_filtrado.columns:
        ...
        if df_pivot_pontos.empty:
            st.info("Nenhuma pontuação encontrada para o período selecionado para exibir a evolução mensal.")
        
        # 2. IDENTIFICA AS ÚLTIMAS DUAS TEMPORADAS SELECIONADAS (USA NOVA FUNÇÃO)
        temporadas = get_last_two_seasons(temporadas_selecionadas_exib)
        
        # 5. Cálculo da Evolução em Porcentagem (Apenas se houver 2 ou mais selecionadas)
        if temporadas:
            
            t_atual_col, t_anterior_col, t_atual_tx, t_anterior_tx = temporadas
            
            nome_coluna_evolucao = f"Evolução Pontos ({t_atual_tx} vs {t_anterior_tx})"
            ...
            st.markdown(f"**Nota:** A coluna de Evolução Pontos compara o crescimento mensal entre a **{t_atual_tx}** (Atual) e a **{t_anterior_tx}** (Anterior) seleccionadas.")
    ...
    if not df_segmento_metrica.empty:
        ...
        st.info("Nenhum dado encontrado para a distribuição por segmento.")
    ...
    if 'Data da Venda' in df_base_tendencia.columns and not df_base_tendencia.empty:
    ...
    elif 'Data da Venda' in df_base_tendencia.columns:
        st.info("Nenhum dado de vendas encontrado para o período selecionado no filtro lateral.")


    st.markdown("---")
    # 6. A. Pedidos Únicos por Mês
    if 'Mês_Exibicao' in df_base_tendencia.columns and COLUNA_PEDIDO in df_base_tendencia.columns and not df_base_tendencia.empty:
    ...
    if 'Mês_Exibicao' in df_filtrado.columns and 'Temporada_Exibicao' in df_filtrado.columns and COLUNA_PEDIDO in df_filtrado.columns:
        ...
        if not df_pivot_pedidos_display.empty:
            st.dataframe(
                df_pivot_pedidos_display.style
                    ...
                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, colunas_temporada_tx]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                column_config=colunas_config_pedidos,
                use_container_width=True,
            )
        else:
            st.info("Nenhum pedido encontrado para o pivô mensal.")
        ...
        if meses_detalhe and temporadas_detalhe:
            ...
            # CRÍTICO: CORREÇÃO AQUI - Desformata o valor numérico da célula para cálculo
            valor_celula_str = str(df_pivot_pedidos_display.loc[mes_selecionado_exib, temporada_selecionada_tx])
            
            # Remove a formatação de milhar (ponto) e troca a vírgula por ponto para conversão
            valor_numerico_limpo = valor_celula_str.replace('.', '').replace(',', '.')
            valor_numerico = pd.to_numeric(valor_numerico_limpo, errors='coerce')
            ...
            if valor_numerico > 0:
                ...
                fig_detalhe.update_layout(xaxis_title="Segmento", yaxis_title="Pedidos Únicos")
    ...
    # 1. IDENTIFICA AS ÚLTIMAS DUAS TEMPORADAS SELECIONADAS (USA NOVA FUNÇÃO)
    temporadas = get_last_two_seasons(temporadas_selecionadas_exib)

    if temporadas:
        
        # T_Atual e T_Anterior são as duas últimas selecionadas
        t_atual_nome, t_anterior_nome, t_atual_tx, t_anterior_tx = temporadas
        ...
        if not df_segmento_evolucao.empty:
            st.dataframe(
                df_segmento_evolucao.style
                    ...
                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, [col_pontos_atual, col_pontos_anterior, 'Evolução %', 'Participação %']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                use_container_width=True
            )
        else:
            st.info("Nenhum dado encontrado para a evolução de pontuação por segmento.")
    ...
    if temporadas:
        ...
        # Garantir que se o filtro de loja estiver vazio, o cálculo não falhe
        ...
        if not df_display_evolucao.empty:
            st.dataframe(
                df_display_evolucao.style
                    ...
                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['Evolução %']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                use_container_width=True
            )
        else:
            st.info("Nenhum dado de loja encontrado para o comparativo T vs T-1.")
        ...
        if not df_rank_quantitativo.empty:
            st.dataframe(
             ...
             st.info("Nenhum dado encontrado para o ranking quantitativo de lojas.")
        ...
        if not df_rank_pontuacao.empty:
            st.dataframe(
            ...
            st.info("Nenhum dado encontrado para o ranking de pontuação acumulada.")
        ...
        if not df_piramide_sumario.empty:
            st.dataframe(
            ...
            st.info("Nenhum dado encontrado para o sumário da pirâmide de evolução.")
    ...
    if COLUNA_ESPECIFICADOR in df_filtrado.columns:
        ...
        # 1. NOVO: CALCULA O DATAFRAME BASE CONSOLIDADO (USANDO CACHE)
        # Passamos a base já filtrada por Segmento/Loja/Mês/Temporada (df_filtrado)
        df_desempenho = calcular_desempenho_consolidado(df_filtrado, temporada_selecionada_t8)
        
        if df_desempenho.empty:
            st.info("Nenhum dado de desempenho consolidado encontrado para os filtros selecionados.")
            # Sai do bloco do Item 8 se não houver dados
            st.markdown("---")
            # Continua na próxima análise (Item 9)
        else:
            # Pega o número da temporada atual calculado dentro da função cacheada
            temporada_atual_num_t8 = df_desempenho['Temporada_Atual_Num'].iloc[0]
            temporada_anterior_num_t8 = int(temporada_atual_num_t8) - 1 if int(temporada_atual_num_t8) > 0 else 0
            
            # --- O CÓDIGO DE AGRUPAMENTO DE PONTUAÇÃO E CATEGORIZAÇÃO FOI REMOVIDO DAQUI ---
            # --- AGORA CONTINUA COM A LÓGICA DE EVOLUÇÃO E DISPLAY ---
            ...
            pontuacao_total_atual = df_desempenho['Pontuacao_Total'].sum() # Usa o total do df_desempenho
            ...
            # CÓDIGO CSS PARA O AUMENTO DA PORCENTAGEM (EVOLUÇÃO) DO ITEM 8
            st.markdown("""
                <style>
                    /* Altera a fonte do texto de Evolução (e outros textos marcados com a classe p) */
                    div[data-testid="stVerticalBlock"] p {
                        font-size: 22px; /* MUDANÇA: Ajustado para o STYLE_FONT_SIZE */
                    }
                    /* Centraliza os títulos das categorias no topo da matriz */
                    div[data-testid="stVerticalBlock"] > div > div > div:nth-child(1) p {
                    }
                </style>
                """, unsafe_allow_html=True)
            ...
            for i, categoria in enumerate(colunas_matriz_display):
                ...
                with colunas_kpi_contagem[i]:
                    # MUDANÇA: Aumentado para 18px (via CSS Global no bloco acima)
                    st.markdown(f"<p style='color: {cor}; font-weight: bold;'>{categoria}</p>", unsafe_allow_html=True)
            ...
            for i, categoria in enumerate(colunas_matriz_display):
                ...
                with colunas_kpi_evolucao[i]:
                    ...
                    # MUDANÇA: Aumentado para 18px (via CSS Global no bloco acima)
                    st.markdown(f"<p style='color: {cor}; font-weight: bold;'>{evolucao_texto}</p>", unsafe_allow_html=True)
            ...
            temporada_anterior_nome = f"Temporada {temporada_anterior_num_t8}"
            ...
            # Exibição Final
            if not df_tabela_exibicao.empty:
                st.dataframe(
                    df_tabela_exibicao.style
                        ...
                        .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['Pontuação', 'Qtd de Pedidos', 'Categoria', 'Evolução T vs T-1']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                    use_container_width=True
                )
            else:
                st.info("Nenhum dado encontrado para o desempenho individual.")
            ...
            **Nota:** A coluna **Evolução T vs T-1** compara a pontuação consolidada da Entidade na temporada atual selecionada ({temporada_selecionada_t8}) com a pontuação que ela teve na temporada anterior ({temporada_anterior_nome}) **dentro do mesmo Segmento/Loja filtrado**.
            ...
            # =======================================================================
            # ITEM 9. ANÁLISE DE NOVOS CADASTRADOS (Aquisição)
            # =======================================================================
            if COLUNA_ESPECIFICADOR in df_filtrado.columns:
                ...
                df_novos_filtrados = df_filtrado.copy()
                ...
                for i, t_col in enumerate(colunas_temporada_str):
                    # Filtra pela NOVA lógica de coorte (apenas vendas T=T_Cadastro)
                    df_temp_novos = df_novos_filtrados[
                        (df_novos_filtrados['Temporada_Exibicao'] == t_col) &
                        (df_novos_filtrados['Novo_Cadastrado'] == True)
                    ]
                    
                    pontos = df_temp_novos['Pontos'].sum()
                    contagem_clientes = df_temp_novos['CNPJ_CPF_LIMPO'].nunique()
                ...
                if 'Mês_Exibicao' in df_novos_filtrados.columns:
                    ...
                    # 1. Agrupamento e Cálculo do Pivô - USA FUNÇÃO IMPORTADA (passamos df_novos_filtrados que já contém a coorte)
                    ...
                    if not df_pivot_novos.empty: # Adiciona um último check antes de renderizar
                        st.dataframe(
                            df_pivot_novos[colunas_display_final_cli].style.format({col: formatar_milhar_br for col in colunas_clientes_renomeadas + ['Total']})
                                                                    ...
                                                                    .set_properties(**STYLE_TABLE_PROPS), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                            use_container_width=True
                        )
                    else:
                    ...
                    termo_pesquisa_9 = st.text_input(
                        "Pesquisar Nome/CPF/CNPJ:",
                        key='search_item_9',
                        placeholder="Digite o nome, CPF ou CNPJ para filtrar a tabela de Ativados e Cadastrados"
                    )
                    
                    # Agrupa os novos clientes que FORAM ATIVADOS (Novo_Cadastrado == True)
                    df_ativados = df_novos_filtrados[df_novos_filtrados['Novo_Cadastrado'] == True].copy()
                    
                    if not df_ativados.empty:
                        df_nomes_novos = df_ativados.groupby(
                            [COLUNA_ESPECIFICADOR, 'CNPJ_CPF_LIMPO', 'Temporada_Cadastro_Original']
                        ).agg(
                            Primeira_Compra_Historica=('Data da Venda', 'min'),
                            Qtd_Pedidos=(COLUNA_PEDIDO, 'nunique') # Quantidade de pedidos no período filtrado
                        ).reset_index()
                        
                        # Adiciona o CNPJ/CPF original (Para Masking)
                        df_cnpj_original_map = df_dados_original.groupby('CNPJ_CPF_LIMPO').agg({
                            COLUNA_CNPJ_CPF: 'first' 
                        }).reset_index().rename(columns={COLUNA_CNPJ_CPF: 'CPF/CNPJ_Original'})
                        
                        df_nomes_novos = pd.merge(df_nomes_novos, df_cnpj_original_map, on='CNPJ_CPF_LIMPO', how='left')
                        # AGORA USA A FUNÇÃO IMPORTADA: formatar_documento
                        df_nomes_novos['CPF/CNPJ_Mascarado'] = df_nomes_novos['CPF/CNPJ_Original'].apply(formatar_documento)
                        
                        # Renomeia e formata
                        df_nomes_novos.columns = ['Nome', 'CNPJ_CPF_LIMPO', 'Temporada_num', 'Primeira Compra (Período)', 'Qtd Pedidos', 'CPF/CNPJ Original', 'CPF/CNPJ Mascarado']
                        df_nomes_novos['Temporada'] = 'T' + df_nomes_novos['Temporada_num'].astype(str)
                        # NOVA COLUNA: Já Comprou (agora é sempre 'Sim', mas mantemos para consistência)
                        df_nomes_novos['Já Comprou'] = 'Sim'
                        
                        df_nomes_novos['Primeira Compra (Período)'] = df_nomes_novos['Primeira Compra (Período)'].dt.strftime('%d/%m/%Y')
                        
                        # --- APLICAÇÃO DO FILTRO DE PESQUISA (COMPARTILHADO) ---
                        if termo_pesquisa_9:
                            termo_pesquisa_lower = termo_pesquisa_9.lower()
                            # Inclui a pesquisa no CPF/CNPJ limpo (não mascarado) para ser mais robusto
                            df_nomes_novos = df_nomes_novos[
                                df_nomes_novos['Nome'].astype(str).str.lower().str.contains(termo_pesquisa_lower, na=False) |
                                df_nomes_novos['CPF/CNPJ Mascarado'].astype(str).str.lower().str.contains(termo_pesquisa_lower, na=False) |
                                df_nomes_novos['CPF/CNPJ Original'].astype(str).str.lower().str.contains(termo_pesquisa_lower.replace('.', '').replace('-', '').replace('/', ''), na=False) 
                            ]
                            
                        # Colunas finais para exibição (reordenadas e renomeadas)
                        df_display_9B = df_nomes_novos[[
                            'Nome', 'CPF/CNPJ Mascarado', 'Primeira Compra (Período)', 'Temporada', 'Já Comprou'
                        ]].rename(columns={'CPF/CNPJ Mascarado': 'CPF/CNPJ'})
                        ...
                        if not df_display_9B.empty:
                            st.dataframe(
                                df_display_9B.style
                                    .applymap(style_ja_comprou, subset=['Já Comprou'])
                                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['CPF/CNPJ', 'Primeira Compra (Período)', 'Temporada', 'Já Comprou']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                                use_container_width=True
                            )
                        else:
                            st.info("Nenhum novo profissional ativado encontrado com os filtros aplicados e a lógica de coorte (T de Venda = T de Cadastro).")
                    ...
                    # 2. Adiciona o status de compra à base de Novos Cadastrados Originais (df_novos_cadastrados_original)
                    if not df_novos_cadastrados_original.empty:
                        df_cads_temp = df_novos_cadastrados_original.copy()
                        df_cads_temp['CPF_LIMPO'] = df_cads_temp[COLUNA_CPF_NOVO_CADASTRO].astype(str).str.replace(r'[^0-9]', '', regex=True)
                        
                        # 3. Mapeia a ativação
                        df_cads_temp['Já Comprou'] = np.where(
                            df_cads_temp['CPF_LIMPO'].isin(clientes_ativados_coorte_limpos), 'Sim', 'Não'
                        )
                        
                        # 4. Formata o CPF Mascarado
                        # AGORA USA A FUNÇÃO IMPORTADA: formatar_documento
                        df_cads_temp['CPF/CNPJ Mascarado'] = df_cads_temp[COLUNA_CPF_NOVO_CADASTRO].apply(formatar_documento)
                        
                        # 5. Filtro de Pesquisa (Compartilhado com 9B)
                        if termo_pesquisa_9:
                            termo_pesquisa_lower = termo_pesquisa_9.lower()
                            # Inclui a pesquisa no CPF/CNPJ limpo (não mascarado) para ser mais robusto
                            df_cads_temp = df_cads_temp[
                                df_cads_temp['Nome'].astype(str).str.lower().str.contains(termo_pesquisa_lower, na=False) |
                                df_cads_temp['CPF/CNPJ Mascarado'].astype(str).str.lower().str.contains(termo_pesquisa_lower, na=False) |
                                df_cads_temp[COLUNA_CPF_NOVO_CADASTRO].astype(str).str.lower().str.contains(termo_pesquisa_lower.replace('.', '').replace('-', '').replace('/', ''), na=False)
                            ]
                        
                        # 6. Colunas Finais para Exibição
                        # Verifica se E-mail e Telefone existem antes de incluir
                        colunas_display_9C_base = ['Nome', 'CPF/CNPJ Mascarado', 'Temporada', 'Já Comprou']
                        if 'E-mail' in df_cads_temp.columns: colunas_display_9C_base.insert(2, 'E-mail')
                        if 'Telefone' in df_cads_temp.columns: colunas_display_9C_base.insert(3, 'Telefone')

                        df_display_9C = df_cads_temp[colunas_display_9C_base].rename(columns={'CPF/CNPJ Mascarado': 'CPF/CNPJ'})
                        ...
                        if not df_display_9C.empty:
                            st.dataframe(
                                df_display_9C.style
                                    .applymap(style_ja_comprou_9C, subset=['Já Comprou'])
                                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['CPF/CNPJ', 'Temporada', 'Já Comprou']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                                use_container_width=True
                            )
                        else:
                            st.info("Base de Novos Cadastrados não carregada ou vazia com os filtros de pesquisa aplicados.")
                            
                    else:
                        st.info("Base de Novos Cadastrados não carregada ou vazia.")
                ...
                if not df_dados_original.empty:
                    ...
                    if not df_anual_metricas.empty:
                        evento = st.dataframe(
                        ...
                        st.info("Nenhuma métrica anual de retenção encontrada para os filtros de entidade.")
                        evento = None # Garante que a lógica de clique abaixo não falhe
                    ...
                    # Formata o CPF/CNPJ Mascarado para exibição
                    # AGORA USA A FUNÇÃO IMPORTADA: formatar_documento
                    df_detalhe['CPF/CNPJ Mascarado'] = df_detalhe['CPF/CNPJ Original'].apply(formatar_documento)
                    
                    # Limpa colunas
                    df_detalhe.drop(columns=['CPF/CNPJ_LIMPO', 'CPF/CNPJ Original'], inplace=True)
                    df_detalhe.rename(columns={'CPF/CNPJ Mascarado': 'CPF/CNPJ'}, inplace=True)
                    ...
                    if not df_tabela_detalhe_exibicao.empty:
                        st.dataframe(
                            df_tabela_detalhe_exibicao.style
                                ...
                                .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, ['Status', 'Qtd de Pedidos no Período', 'Última Data da Compra']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                            use_container_width=True
                        )
        ...
        if df_ranking_ajustado.empty:
            ...
            st.dataframe(
                df_display.style
                    ...
                    .set_properties(**STYLE_TABLE_PROPS, subset=pd.IndexSlice[:, [col_t_anterior, col_t_atual, 'Variação Rank']]), # APLICA ESTILO GLOBAL (TAMANHO + CENTRALIZAÇÃO)
                use_container_width=True
            )
